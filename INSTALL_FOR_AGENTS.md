...
你要做的事，按顺序：**看前置（§2）→ 准备实例仓库（§3）→ 部署到 Railway（§4）→ 配 TOKENS_JSON（§5）→ 接客户端（§6）→ 跑自验（§7）**。

---

## 2. 形态与前置

**本版只有「云」这一条路径。** 主人自己维持云（Railway 已是够得着手机 + 远程 fleet 且不架隧道的形态）。纯本地单机的「60 秒试用」入口留到 M4.5 打包时再给——现在别自己发明本地路径。
...
以 `service/src/server.js` 入口块为准（15 个）。**必填只有 `REPO_URL` 和 `TOKENS_JSON`**，其余都有安全默认。
