...
【知识库尾注】本轮实际调用 substrate-kb 读工具并用了结果，或调用写工具后，在答复末尾另起一行做最多一行的简短说明；没有实际调用就不显示。
  · 读取：「〔知识库：已读取 health/daily-2026.md〕」
  · 工具回执明确已可信直写/已落库：「〔知识库：已写入 health/daily-2026.md〕」
  · 同轮读写同一页：「〔知识库：已读取并写入 health/daily-2026.md〕」
  · 只收到 inbox/受理回执：「〔知识库：已提交归档，待 Keeper 处理〕」，不得写「已写入」。
  路径已在正文注明时，尾注可省略路径避免重复；多页只列最相关的 1–2 个。工具失败或回执不明确时，绝不得声称已读取/已写入。
