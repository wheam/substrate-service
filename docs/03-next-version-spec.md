...
- **`save.hint` 的稳定页寻址**：更新现有页可明确给 `path: <实例相对路径>` 或 `content_id: <8 位 id>`。keeper 代码先在注册 zone 内唯一定位真实路径，再执行模型决定；零命中、撞 id、path/id 冲突一律 held，不按 `<zone>/<slug>.md` 猜。该确定性覆盖只认本进程亲生件或已认证主人裁定，git pull 伪造件不能借 hint 强制改页。完整 canonical Skill 文档按 `skills/<name>/SKILL.md` 原位替换并保留旧 `content_id`；新 Skill 只允许先写 `skills/_incoming/<name>/SKILL.md`，递归创建父目录，不生成扁平页。
...
- **M4.9.1 核心记忆投影修复（2026-07-14，本机 Claude Opus 审查 + Codex 实现）**：真漏点是 v1 `substrate-memory` 的「分类页写后重校准 `_core.md`」没有迁到 v2 keeper；`/digest` 只整段注入 `_core.md`，分类页正文只进目录，所以稳定偏好虽已入库，Hermes 的常驻小抄仍看不到。修复把 `_core.md` 定义为 **canonical about-owner 分类页的特权派生投影**：独立 `core-calibration` worker 按 source hash 检测变化（不污染纯确定性的 nightly），模型只产固定 section/bullet JSON，代码固定模板、NFKC/隐形字符/上下文注入扫描、正文 `≤3000` 硬闸；生成 `kind:core` held 提案，只有 `trust:high + channel:primary` 的主人点选后才可走 `calibrate_core` 整页原子替换。普通 `new_page/merge_into` 在执行器层禁止碰 `README`/`_*` 结构页，彻底封住原先把 raw capture 追加进 always-load core 的入口。提案全文享专用 4k 预览但仅对当前 native 件放宽；重启后失去 native 登记的旧提案会被行政关闭再重建，避免永久卡死；拒绝同一 source 后不骚扰式重提，来源或 core 漂移才再校准。
