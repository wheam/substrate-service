...
- 语义索引默认开的阈值：落空率到多少才值得引入 embedding 依赖？（M4.1 用 M4.0 的数据回答，仍开放。2026-07-06 基线补充：M4.x 上线后 `search`/`recall` 零调用、读侧大头是 `collections_search`——此问暂无数据可答且不急；读侧真实痛点见 Hermes 截断案例，候选 M4.7。）
