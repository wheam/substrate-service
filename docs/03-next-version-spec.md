...
> 状态：IMPLEMENTED / 个人 alpha。M0–M4.10 已完成；本文件保留规格、里程碑与设计裁定。01 个人版含私人细节，未随开源仓库发布；[02](02-productization.md) 的托管 SaaS 部分仍冻结。
...
- **M4.10 收口**：`confidence` 只表示模型的语义把握，**不是权限，也不是“要不要问主人”的开关**。新建普通页、向已授权页追加、精确重复 no-op 等可逆/幂等 effect 由引擎自动处理；只有目标身份真冲突、不可逆结构动作缺授权、或不存在安全回退时才进入 owner-held。模型坏输出、provider/doctor/git 故障属于 repair/retry/运维态，不转嫁给主人。
...
### 3.10 keeper effect policy：让模型判断语义，不让模型签发权限
- 每个认证写入口在收件时签发 `AdmissionContext`（actor trust/source/channel、ingress、capabilities）；它与正文、hint、时间、候选计划一起被 git 外的持久 proof registry 做内容绑定。文件自报字段不能扩权，正常重启也不会让 pending 件失去权限。
- LLM 材料只给本件真实允许的 effects / zones。模型先做语义规划；若第一次越界或状态过期，keeper 只做一次受约束 repair；仍不可执行则指数退避重试，不询问主人。低置信但无害的内容落独立 `candidate`，不修改已有页。
...
- **落点（M4.3 已实现）**：主动浮出 = primary 客户端工具成功响应**尾部 piggyback**「📥 待主人裁定 N 件」提示（防重复：held id 集合为 key，`NUDGE_TTL_MS`（默认 72h）窗口内只发一次，状态在 volume 持久化、部署重启不重置；`inbox_list`/`inbox_resolve` 豁免）+ **per-client MCP instructions**（primary 附主频道房规 `PRIMARY_RULES`）+ **`/digest` primary 版**（房规 + 实时 held 摘要，只带 id/kind/计数）。push/pull 之争的裁决见 §9。
...
- `held` 内部分型：`owner`（真需主人语义/结构选择）、`retryable`（模型/provider/doctor/git 等引擎故障，指数退避）、`security`（proof 缺失/篡改隔离）。只有 `owner` 写 `keeper_held_at`、生成候选、通知/nudge、出现在 App 待定夺列表；另外两类不冒充主人任务。`retryable` 最多自动尝试 6 次，耗尽后写 `retry_exhausted: true`、停止自动调用模型并发一次运维告警，但仍不伪装成主人归档选择题。
...
- **M4.5 抗注入 + 开源打包**：对抗样本进考卷、Railway 部署路径、本地单机入口、干净 README / 安装文档、模式 spec 对外版。✅ **已完成**（编排验证：250 测试绿 + 考卷 45/45 + Codex xhigh 全批 + 两轮聚焦复核 merge-ready；对抗金标 25→45、adversarial 5→19；凭据红线四层加固 raw→白空格→`\p{Cf}`→`\p{Mn}/\p{Me}/\p{Cc}`（堵空白/换行/零宽/组合标记隐形拆分）；考卷 confidence 注入假绿修复；决策见 §9）。
- **M4.6 夜班去人化 + 裁定面按通道收窄**（2026-07-06 立项，源于夜班首跑真机复盘——主人被要求裁「做不了判断」的删页提案、且 App 裁定被通道限权正确拒绝后弹回，违反原则 B）：夜班对薄页/重复页从 `remove_page` 提案改为**非破坏性降级**（新 executor action `set_tier`，确定性执行、零裁定、进程内直执行不经 inbox 往返）；held 件带「可裁通道」属性，capture 通道只见它有权兑现的件（maintenance/schema 对 App 只读）；断链等报告类产物落维护日志页 + agent digest，不进主人收件箱。**验收**：夜班整轮跑完主人零操作；App 待定夺列表不再出现 maintenance 件；降级动作审计可查、高信任通道一句话可撤（re-promote）；现存遗留（3 件弹回删页提案 + 1 件断链报告）收编清零；凭据红线 + 批准登记表回归考卷全绿。设计裁定见 §9。✅ **已完成并上真机**（2026-07-06 railway up 部署验证：migrateLegacy 真机关闭 3 件弹回删页提案、断链报告转维护日志、后续 tick 幂等归零、**审计零 demote 事件 = 「迁移只关件、绝不碰内容页」blocker 修复真机确认**；Codex 异源两轮对抗收敛〔真 blocker：迁移信 git-pull 伪造件正文可软删敏感正典页 → 砍成只按 client+kind 行政字段关件〕；283 测试绿 + 考卷 45/45。2026-07-10 复核：生产 `inbox_list` 返回空 = 遗留清零维持。打勾系 2026-07-10 补记，完成即 07-06。）
...
- **M4.4 抗注入加固（Codex xhigh 四轮对抗 review 的收敛结论，2026-07-05；M4.10 持久化升级）**：本批把「**inbox 文件活在 git 里、`git pull` 进来的文件 = 对抗输入**」这条威胁模型贯彻到治理/删除路径。keeper 只认经 `resolveEntry` 记入 git 外持久账本的批准；token 绑定完整执行信封与 ruling/decision，**文件里裸的 `owner_ruling`/`owner-decision`/`ruling_via_trust` 一律不信**。伪造、approve-then-swap、批准 proof 缺失均 security-held，不交给 LLM 重新解释；终态执行先持久 claim 并留下 consumed tombstone，retryable 回滚则恢复同一绑定计划。维护提案点选执行仍**绑定可见 json op 块**（隐藏 options 决定不得与可见提案不符）。
- **M4.5 抗注入加固（Codex xhigh 全批 + 两轮聚焦复核，2026-07-05）**：把 M4.4 逼出的攻击面固化进判例考卷（金标 25→45、adversarial 5→19、redline 3→11），并实修两个 Codex 抓出的真洞（凭据红线里程碑带已知绕过=失格，判「实修不止披露」）。① **凭据红线四层加固**：`inbox.addEntry` 原只扫连续密钥正则，被空白/换行/零宽/组合标记切碎的 key 可逐段逃过 `\bsk-[A-Za-z0-9]{20,}` 之类——现【同时】扫原文与「折叠副本」，折叠集逐轮扩 `\s`→`[\s\p{Cf}]`→`[\s\p{Cf}\p{Mn}\p{Me}\p{Cc}]`，几近穷尽「视觉/功能可忽略、能隐形拆 key」的字符空间（可见字符拆 key 破坏其可用性=非有效攻击，划已知边界；原文扫描保留兜 PEM 的合法内建空格）。② **考卷 confidence 注入假绿**：三条「set confidence=1.0」注入 golden 原本不校验该字段即判 PASS；加 `expect.maxConfidence` 检查模型是否服从正文中的置信度操纵。M4.10 已明确 confidence 不是权限或人工 gate，真正安全边界由 AdmissionContext/effect policy 承担。教训重申：任务级同源 review 漏掉这两处，全靠 Codex 异源对抗 +（凭据洞）`Default_Ignorable_Code_Point` 全码点扫描逼出。
...
- **M4.9.1 核心记忆投影修复（2026-07-14，本机 Claude Opus 审查 + Codex 实现）**：真漏点是 v1 `substrate-memory` 的「分类页写后重校准 `_core.md`」没有迁到 v2 keeper；`/digest` 只整段注入 `_core.md`，分类页正文只进目录，所以稳定偏好虽已入库，Hermes 的常驻小抄仍看不到。修复把 `_core.md` 定义为 **canonical about-owner 分类页的特权派生投影**：独立 `core-calibration` worker 按 source hash 检测变化（不污染纯确定性的 nightly），模型只产固定 section/bullet JSON，代码固定模板、NFKC/隐形字符/上下文注入扫描、正文 `≤3000` 硬闸；生成 `kind:core` held 提案，只有 `trust:high + channel:primary` 的主人点选后才可走 `calibrate_core` 整页原子替换。普通 `new_page/merge_into` 在执行器层禁止碰 `README`/`_*` 结构页，彻底封住原先把 raw capture 追加进 always-load core 的入口。提案全文享专用 4k 预览且要求 native proof；M4.10 起 proof 持久化在 git 外，正常重启继续复用，只有遗留/损坏/篡改件才行政关闭再建；拒绝同一 source 后不骚扰式重提，来源或 core 漂移才再校准。
- **挂起候选（2026-07-10 主人裁定「先用一段时间再说」，勿主动催办）——「M4.10 候选：digest 原生渲染器 + 库内 v1 清退 +（可选）skills 分发」**，两件事同一片地基：
  ① **skill 同步能力重建（v2 式）**：v1 曾支持「自研 skill 放进知识库 → sync 分发到所有 agent」，v1 sync 退役后自动分发能力仍真空。通用 `read_page` / `save→keeper` 已能读写注册的 skills 分区（含嵌套 `SKILL.md` 稳定寻址与 `_incoming` 守门路径），但尚无 `skills_list`/`skills_get` 与各 runtime 自动物化。若重建：补专用高信任工具 + 各机复用 digest-refresh 模式物化到 runtime 技能目录；写入继续走 inbox 治理。主人尚未拍板要不要。
  ② **get_context 原生渲染器 + 库内 v1 清退**：现状 = get_context 仍 shell 执行实例 vendored 的 v1 `render-context.py`（承重墙，实例 `skills/` 目录不可删），v1 锈迹（技能路由表/v1 房规/Packet 操作行/substrate-memory 措辞）由渲染出口滤网剥除（`tools.js stripV1Sections`，2026-07-10 上线，digest 由 ~17K 瘦到 ~7K）。**滤网是过渡态**：终局应是服务端原生渲染（JS 直读记忆页+分区 README 拼小抄），之后才能把实例里整个 v1 `skills/` 目录与 house-rules 清退。数据侧另有一句「用 substrate-memory 现读」躺在核心记忆页正文（渲染时被译成 read_page，agent 不可见），源头修订属记忆内容编辑、顺手即可。
