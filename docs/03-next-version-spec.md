...
- **挂起候选（2026-07-10 主人裁定「先用一段时间再说」，勿主动催办）——「M4.10 候选：digest 原生渲染器 + 库内 v1 清退 +（可选）skills 分发」**，两件事同一片地基：
  ① **skill 同步能力重建（v2 式）**：v1 曾支持「自研 skill 放进知识库 → sync 分发到所有 agent」，v1 sync 退役后此能力真空（v2 工具清单无 skill 读写）。若重建：库内立 skills 分区 + 服务端 `skills_list`/`skills_get`（高信任档）+ 各机复用 digest-refresh 模式物化到 runtime 技能目录，写入照走 inbox 治理。主人尚未拍板要不要。
